...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let body: { name?: string; provider?: string; instance_name?: string; uazapi_url?: string; uazapi_token?: string; active?: boolean };
  ...
  if (typeof body.provider === "string" && ["uazapi", "evolution"].includes(body.provider)) patch.provider = body.provider;
  if (typeof body.instance_name === "string") patch.instance_name = body.instance_name.trim() || null;
